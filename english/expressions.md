...
### 원서로 스피킹 연습?
  ...
  1. 평소 읽듯이 원서를 읽는다.
  2. 방금 읽은 문장이 어떤 내용인지 떠올린다.
  3. 영어로 말해본다.

  위 연습과정이 도움이 될 것이라 생각하는 이유는, 읽기는 문자를 내 머리 속의 무언가로 바꾸는 과정이고, 말하기는 내 머리 속의 무언가를 끄집어 내는 것이다. 무작정 말하기를 연습하자니, 무언가를 다짜고짜 생각해내기가 어렵고 매번 똑같은 생각을 하기 마련이다. 하지만, 위 과정을 진행하면 자연스레 무언가를 생각해 내는 과정은 필요가 없으며, 단지 그 무언가에 대한 느낌을 어떻게 표현할지에 대해 집중할 수가 있게 된다. 읽기는 물론 하던대로, 문자를 그 무언가로 빠르게 변환하는 훈련이 자연스레 될 것이다. 현재는 말하기 실력이 읽기 실력에 비해 현저히 떨어지므로 읽기에 대해서는 그리 문제가 되지 않을 것이다.
